...
describe("Testing DAO", async function () {
  ...
  it("Testing set votes duration", async()=>{
    ...
    expect(await dao.vote_duration()).to.equal(proposal_duration);
    await dao.connect(owner).setVoteDuration(new_duration);
    expect(await dao.vote_duration()).to.equal(new_duration);
  })
  ...
  it("Testing vote function", async()=>{
    ...
    expect(String(prop.resolve_votes)).to.equal(my_votes);
  })
...
///////////////
  console.log("Trying to finish while `resolve_votes` equals to `reject_votes`!");
...
//////////////
  ...
  await dao.connect(user3).deposit(String(Number(my_votes)/4));

  await increaseTime(proposal_duration);

  await dao.connect(owner).deposit(String(Number(my_votes)*8));
  await dao.connect(owner).vote(1,true);
